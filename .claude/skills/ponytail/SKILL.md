...
Look for what a mechanical check cannot see in the change itself:
...
- A check that could not run is `SKIP`, never assumed green.
- A number you estimated is labelled estimated.
- A conclusion resting on an unverified assumption names it (UNKNOWN = FAIL).
