...
1. **Server state** — fetched, cached, refetched, can go stale. Belongs to a data layer
   (a loader, a query cache), never to ad-hoc local state.
2. **URL state** — filters, tab, page, sort. Belongs in the URL so it survives reload
   and can be shared. Reaching for local state here is the most common mistake.
...
4. **Form state** — the library the project already uses, with schema validation
   (`zod`) shared with the server's own check.
...
- A context/store that changes often re-renders every consumer — split by update
  frequency.
- Keys come from data identity, never the array index.
- Every effect that subscribes, times, or fetches returns its cleanup.
- Memoize after a profiler says so, not before.
...
LCP, CLS and INP are the ones users feel — set a budget before optimising.
