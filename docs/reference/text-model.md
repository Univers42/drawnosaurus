...
A click on a text that was already the sole selection opens the caret where it landed; the
same click on a shape's label does too, but only when the label itself — not merely
anywhere on the shape — is what the press landed on: the topmost thing under it, a bound
label counted, must resolve to the label
(`getSelectedTextEditingContainerAtPosition`, `getTextElementAtPosition`,
`App.tsx@1118751f:6551-6582`). Every other entry point — Enter, a double-click, a click
elsewhere on the shape, or a click that changes what is selected — opens with the whole
text selected, as it always did (`getCaretIndexFromInitialSceneCoords`,
`App.tsx@1118751f:12402-12428`, `textWysiwyg.tsx@1118751f:488-555`; `ci_text_edit.rs` ›
entry; `e2e/textEditor.spec.ts` › a click on a text that was already the sole selection). A
fixed-width text shows the dashed box it wraps in while it is typed; a label's own shape
outline already does that job, and an auto-sizing text has none (`renderTextBox`,
`interactiveScene.ts@1118751f:1510-1531`, `:1661-1672`; `e2e/textEditor.spec.ts` › a
fixed-width text shows its box outline). A new text made with the grid on lands on it
(`getTextCreationGridPoint`, `App.tsx@1118751f:7008`; `ci_text_edit.rs` › a new free text
snaps to the grid; `e2e/textEditor.spec.ts` › a new text is snapped to the grid). Deleting a
text made for the edit and never typed into —
