...
Nor do the frame and its handles take a press or offer a cursor, since a press on the board ends
the edit (`pointer.rs` › `selection_framed`; `ci_text_edit.rs` › the hidden frame offers
nothing). Its shape and bound arrows are live and follow it. What is typed goes to peers on the
gesture channel (`gestureElements`), shape included, and their copy of it is refused until the
commit, which stamps above it — a peer's delete meanwhile included; an edit that came to nothing
takes their copy instead (`ci_text_edit.rs` › peers; `e2e/liveSync.spec.ts`). A peer's patch
meanwhile leaves a new label directly above its shape, where their order would put it on top,
and the commit's delta carries the order, so the host's copy — the one saved — is stacked the
same (`ci_text_edit.rs` › a new label keeps its place through a peer's patch). A family only
hovered in the font list is given back before the edit commits, as the oracle's picker puts
back what it cached when it closes (`actionProperties.tsx@1118751f:1483-1500`;
`ci_text_edit.rs` › a family hovered and never picked is not committed,
...
with the tool locked or the autoshape tool (`App.tsx@1118751f:6446-6453`); Ctrl/Cmd+S ends it and
saves the board, as the oracle's does (`textWysiwyg.tsx@1118751f:683-686`); Tab,
Shift+Tab and Ctrl/Cmd+] / [ indent and outdent the selected lines by four spaces; Ctrl/Cmd
with + − 0, without Shift, zoom the board; Ctrl/Cmd+Enter, Ctrl/Cmd+S, Tab and Escape are all
held while an input method composes — none of the four may reach past the composition to
submit, save, indent or end the edit (`:662-711`, `textEditor.test.ts`,
`e2e/textEditor.spec.ts` › Ctrl/Cmd+Enter, Tab and Escape are held). Every other key is the
textarea's, so no letter reaches a tool or a style shortcut, and the font-size chord goes on up
to the board. A press on the
...
A click on a text, or a shape's label, that was already the sole selection opens the caret
where it landed; every other entry point — Enter, a double-click, or a click that changes
what is selected — opens with the whole text selected, as it always did
(`getCaretIndexFromInitialSceneCoords`, `App.tsx@1118751f:12402-12428`,
`textWysiwyg.tsx@1118751f:488-555`; `ci_text_edit.rs` › entry; `e2e/textEditor.spec.ts` › a
click on a text that was already the sole selection). A fixed-width text shows the dashed
box it wraps in while it is typed; a label's own shape outline already does that job, and an
auto-sizing text has none (`renderTextBox`, `interactiveScene.ts@1118751f:1510-1531`,
`:1661-1672`; `e2e/textEditor.spec.ts` › a fixed-width text shows its box outline). A new text
made with the grid on lands on it (`getTextCreationGridPoint`, `App.tsx@1118751f:7008`;
`ci_text_edit.rs` › a new free text snaps to the grid; `e2e/textEditor.spec.ts` › a new text is
snapped to the grid) — see the divergence table below for how the click-to-place path can
still differ from a double-click's. Deleting a text made for the edit and never typed into —
selected, then removed before a keystroke — leaves no step, the same as a new label never
typed into: nothing was ever shown, so there is nothing for undo to bring back
...
| --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| The text stays selected while it is typed, so the style panel keeps working on it; its frame and handles are not drawn and take no press. A style changed meanwhile is written into it and stays with the edit: one step at the commit, nothing stamped before.             | deselects everything (`App.tsx@1118751f:6509-6510`)                                                                           | `ci_text_edit.rs` › painting, peers, style_while_typing; `e2e/textEditor.spec.ts` › a colour picked in the panel; `e2e/fontSize.spec.ts` › one step |
| Deleting lines shrinks the shape back, never below its height when the edit began or when a size or family was last written into the text, which the oracle's editor also takes as its new start (`textPropertiesUpdated`, `textWysiwyg.tsx@1118751f:246-265`, `:326-335`). | shrinks to fit in one step once the shape is taller than it was, which can take it lower (`textWysiwyg.tsx@1118751f:359-373`) | `ci_text_edit.rs` › growth                                                                                                                          |
...
| Escape is held while an IME composes, so it cancels the composition instead of also ending the whole edit out from under it.                                                                                                                                                | lets Escape through unguarded during a composition (`textWysiwyg.tsx@1118751f:627-629`)                                       | `textEditor.test.ts`, `e2e/textEditor.spec.ts` › Ctrl/Cmd+Enter, Tab and Escape are held                                                            |
| A new free text's grid snap floors a point the shared per-gesture snap (`begin_pointer_step`) already rounded, so a double-click and a text-tool click at the same spot can land on different grid points; the oracle's floor only ever sees the raw point.                 | `getTextCreationGridPoint` (`App.tsx@1118751f:7008`)                                                                          | `ci_text_edit.rs` › entry › a new free text snaps to the grid                                                                                       |
| Known limit: Excalidraw JSON pasted into the editor goes in as its own text, not as the elements it names.                                                                                                                                                                  | `textWysiwyg.tsx@1118751f:558-646`                                                                                            | none (gap)                                                                                                                                          |
...
its shape when that shape is not ours. The selection lets in neither a shape a peer holds nor
its label (`peers.rs` › `is_held`), so one is never selected on its own or by Select All, and
one selected before they took the shape is let go — a Delete cannot unbind it from their
shape (`ci_peers.rs` › the label of what they hold is theirs too). The label of a loose
locked shape is neither restyled nor laid out again, and opens no editor — laying it out would grow the
shape (`style.rs` › `restylable`; `ci_text_model.rs` › a shape a peer holds is not grown
through its label, nor by typing into its label). An edit under way when a peer takes the
label or its shape ends, what was typed goes back, and nothing stays selected
(`ci_text_edit.rs` › a peer taking the shape ends the edit, … takes its label out of the
selection). A writer
...
| A label taller than its shape starts at the padded top, whatever its vertical alignment, when only placed (a move, a peer's patch): overflowing downward, it is still read from its first line. The oracle never meets one, because laying a label out grows its shape; here one saved before shapes grew, or placed by a peer's patch from an engine that did not grow them, is placed on every move; a resize grows the shape back to hold its label (`resize.md` › Text and labels). | `computeBoundTextPosition` (`textElement.ts@1118751f:249-324`) centres it | `ci_text_align.rs`, `ci_text_model.rs` › a label taller than its shape |
