...
| a label in the selection            | never there: Select All skips bound text (`actionSelectAll.ts:32-38`), and a click on a label hits its shape, since bound text is not hit (`App.tsx:6726-6736`) | Select All takes labels, and a label moves only with its shape, so a lone label stays where it is. A click or a right-click on a label selects its shape, as the oracle's does, moving both |
