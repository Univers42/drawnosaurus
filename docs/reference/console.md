...
- **Bind text to the container** — exactly a free text and a shape or arrow with no label
  ...
  `:119-127`; `ci_bound_text_actions.rs` › remembered_height);
...
| unbinding a label typed in      | the editor remembers the shape's height when it opens, unless one is remembered already (`wysiwyg/textWysiwyg.tsx:331-345`), so a label typed into a shape and then unbound gives that height back | only a bind remembers one: a shape a typed label grew keeps its growth. The text editor's session is where the height it opened at lives                                                                                                                                                                                                                  |
