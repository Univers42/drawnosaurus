...
the pinned source *and* observed), **INFERRED**, **IMPLEMENTATION DETAIL**, **UNKNOWN**.
...
ordered array, **innermost → outermost**. The array *is* the nesting; there is no separate
...
**VERIFIED** — `addToGroup` (`groups.ts:311-326`) inserts the new id *before*
...
**VERIFIED** — leaving (`App.tsx:9641-9650`): a press on an element *not* in the edited
...
- **State** — `group_ids` on each element (the document); `editing_group_id` on the engine
  ...
  into is a property of *your* view, not of the drawing.
...
- **Derived** — which ids are "selected groups" is derived from the selected elements
  every time; never stored. **IMPLEMENTATION DETAIL:** Excalidraw *does* store
...
**UNKNOWN** — what the reference does when an element is dragged *out* of an edited group,
