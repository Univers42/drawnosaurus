...
**VERIFIED** — `groupIds: readonly GroupId[]` (`packages/element/src/types.ts@1118751f:73`). An
...
**VERIFIED** — `addToGroup` (`groups.ts@1118751f:311-326`) inserts the new id _before_
...
**VERIFIED** — `selectGroupsFromGivenElements` (`groups.ts@1118751f:243-270`). This is the whole of
...
**VERIFIED** — double-click (`App.tsx@1118751f:7312-7336`): when a group is selected and the hit
...
never hit on its own there (`App.tsx@1118751f:6715-6739`), so a double-click on a grouped shape's
...
**VERIFIED** — leaving (`App.tsx@1118751f:9649-9658`): a press on an element _not_ in the edited
group clears `editingGroupId` and the selection.

**VERIFIED** — Escape steps out **one** level, into the group directly around, holding
it; only from the outermost does it let go (`actionDeselect.ts@1118751f:36-62`, `:72-111`).
...
oracle drops it on an empty selection (`App.tsx@1118751f:12897-12910`), a press outside it,
select-all (`actionSelectAll.ts@1118751f:49`) and once the group is gone (`delta.ts@1118751f:806-818`).
...
(`lasso/index.ts@1118751f:72-89`): the loop is resolved at that level on release.
...
a drag and keeps everything (`drag.hasOccurred`, `App.tsx@1118751f:10926-10929`, `:12191-12198`).
...
(`App.tsx@1118751f:9664-9668`, `:12191-12268`). A shift-press on something not held adds it at
...
(`App.tsx@1118751f:6784-6808`, `:9791-9814`), and the cursor says so. A drag from there moves it;
a release with no drag was a click on nothing and lets go of it and of the edited group
(`App.tsx@1118751f:12352-12395`). A line or arrow edited by its points has no box, and so none to
press in (`hasBoundingBox`, `transformHandles.ts@1118751f:328-353`).
...
(`actionDeleteSelected.tsx@1118751f:130-204`). **IMPLEMENTATION DETAIL, deliberate divergence:**
...
(`removeFromSelectedGroups`, `groups.ts@1118751f:327-330`). Observed:
...
- grouping gathers the members directly under the topmost one, in one undo step
  (`actionGroup.tsx@1118751f:170-186`);
- z-order steps over a neighbouring group as one block, and inside an entered group
  moves only within it (`zindex.ts@1118751f:193-299`, `:431-541`);
- a copy made inside the edited group keeps that group and every level around it, with
  ...
  `groups.ts@1118751f:397-413`), and goes directly above the group. **IMPLEMENTATION DETAIL:** the
  oracle puts each copy directly above its own source (`duplicate.ts@1118751f:322-348`); above the
...
- a label carries its shape's groups — when grouped, and when made
  (`App.tsx@1118751f:7083`) — sits directly above its shape (`:7105-7110`), and moves and is
  stepped over with it (`zindex.ts@1118751f:48-51`, `:88-127`). A shape and its own label are
  one thing: they are not grouped with each other, and a group left holding one
  labelled shape is still that group (`actionGroup.tsx@1118751f:73-83`).
...
selected element and refuses only when all are locked (`App.tsx@1118751f:10907-10912`), and its
align, distribute and flip have no lock filter (flip: `resize.md` › Flip).

**VERIFIED** — align and distribute move groups, not elements
(`getSelectedElementsByGroup`, `groups.ts@1118751f:417-466`): each selected group is one block,
...
the two blocks at the ends, which stay (`distribute.ts@1118751f:46-100`). Align needs two blocks,
distribute three, and neither is offered while a frame is selected
(`actionAlign.tsx@1118751f:50-51`, `actionDistribute.tsx@1118751f:43-44`). The inspector asks the engine
...
children, out of any frame, and selects them (`actionDeleteSelected.tsx@1118751f:115-122`);
grouping across a frame's edge takes the group out (`actionGroup.tsx@1118751f:138-150`). Align,
distribute, flip and lock leave membership alone, as the oracle's do outside a drag
(`frame.ts@1118751f:845-855`). A child a peer holds stays put when its frame moves.
...
`App.tsx@1118751f:12072`). An unrelated click never rewrites membership elsewhere — so a child
...
the oracle gives a new element the frame it is created in (`App.tsx@1118751f:10451-10474`,
`App.duplicate.ts@1118751f:124-135`); a pasted or duplicated frame adopts nothing it lands on. A
...
`App.tsx@1118751f:12001-12068`). Each moved member keeps only the groups inside the edited one,
...
(`App.tsx@1118751f:11339-11353`), which holds part of that group beside whole groups outside it —
...
also hands that surgery the selection without its labels (`App.tsx@1118751f:12009`), so a
...
at once, where the oracle waits for the release (`App.tsx@1118751f:9664-9668`). Z-order is
